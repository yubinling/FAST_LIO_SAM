  ...
  if (current_octo->plane_ptr_->is_plane) {
    ...
    if (range_dis <= radius_k * plane.radius) {
      ...
      if (dis_to_plane < sigma_num * sqrt(sigma_l)) {
        ...
        if (this_prob > prob) {
          prob = this_prob;
          single_ptpl.dis_to_plane=dis_to_plane;
...
void BuildResidualListOMP(const unordered_map<VOXEL_LOC, OctoTree *> &voxel_map,
                          ...
                          const std::vector<pointWithCov> &pv_list,
                          std::vector<ptpl> &ptpl_list,
                          std::vector<Eigen::Vector3d> &non_match,int i) {
  std::mutex mylock;
  ptpl_list.clear();
  std::vector<ptpl> all_ptpl_list(pv_list.size());
  std::vector<bool> useful_ptpl(pv_list.size());
...
#pragma omp parallel for
  ...
  for (int i = 0; i < index.size(); i++) {
    ...
    if (iter != voxel_map.end()) {
      ...
      if (is_sucess) {
        ...
        useful_ptpl[i] = true;
        all_ptpl_list[i] = single_ptpl;
        total_distance += single_ptpl.dis_to_plane;
        success_count++;
        mylock.unlock();
      } else {
        mylock.lock();
  ...
  int total_points=pv_list.size();
  if (i==-1){
    float success_ratio=static_cast<float>(success_count) / total_points ;
    double average_dis=total_distance/success_count;
    std::cout << "成功比例: " << success_ratio * 100 << "%" << std::endl;
    std::cout << "成功点面距离的平均值: " << average_dis << std::endl; 
    //fprintf(sf_kitti,"%lf %lf %d\n",success_ratio,average_dis,success_count);
    //fflush(sf_kitti);
    } 
  for (size_t i = 0; i < useful_ptpl.size(); i++) {
    if (useful_ptpl[i]) {
