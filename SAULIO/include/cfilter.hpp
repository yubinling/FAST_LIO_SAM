...
#pragma omp parallel for //Multi-thread
	...
	bool fast_ground_filter(const typename pcl::PointCloud<PointT>::Ptr &cloud_in,
	                    std::vector<pointwithfeature> &sourcecloudwithfeature,
...
#pragma omp parallel for
		for (int i = 0; i < num_grid; i++)
		{
			...
			if (grid[i].pts_count >= min_grid_pt_num && grid[i].reliable_neighbor_grid_num >= reliable_neighbor_grid_num_thre)
			{
				...
				if (estimate_ground_normal_method == 3 && grid_ground->points.size() >= min_grid_pt_num)//地面要大于10
				{
		...
		for (int i = 0; i < cloud_ground->points.size(); i++)
		{
			...
			sourcecloudwithfeature.push_back(pointwithfeature(cloud_ground->points[i],0));
			if (!fixed_num_downsampling)
			{
				...
				if (i % ground_random_down_down_rate == 0)
					...
					sourcecloudwithfeature.push_back(pointwithfeature(cloud_ground_down->points[i],0));
			}
		}
...
#pragma omp section
        {
		for (int i=0;i < cloud_pillar->points.size();i++){
			sourcecloudwithfeature.emplace_back(cloud_pillar->points[i],1);
		}
		for (int i=0;i<cloud_beam->points.size();i++){
			sourcecloudwithfeature.emplace_back(cloud_beam->points[i],2);
		}
		for (int i=0;i<cloud_facade->points.size();i++){
			sourcecloudwithfeature.emplace_back(cloud_facade->points[i],3);

		}

		for (int i=0;i<cloud_roof->points.size();i++){
			sourcecloudwithfeature.emplace_back(cloud_roof->points[i],4);
		}
		}
...
#pragma omp section
		{
		for (int i=0;i < cloud_pillar_down->points.size();i++){
			sourcecloudwithfeaturedown.emplace_back(cloud_pillar_down->points[i],1);
		}
		for (int i=0;i<cloud_beam_down->points.size();i++){
			sourcecloudwithfeaturedown.emplace_back(cloud_beam_down->points[i],2);
		}
		for (int i=0;i<cloud_facade_down->points.size();i++){
			sourcecloudwithfeaturedown.emplace_back(cloud_facade_down->points[i],3);

		}

		for (int i=0;i<cloud_roof_down->points.size();i++){
			sourcecloudwithfeaturedown.emplace_back(cloud_roof_down->points[i],4);
		}
		}
		}
							  ...
							  bool semantic_assisted = false, bool apply_roi_filtering = false, float roi_min_y = 0.0, float roi_max_y = 0.0)
	{
		...
		fast_ground_filter(in_block->pc_down, sourcecloudwithfeature,sourcecloudwithfeaturedown, in_block->pc_ground, in_block->pc_ground_down, in_block->pc_unground, in_block->pc_vertex,
...
# if 0  //ROI_filtering (Deprecated)
		...
		classify_nground_pts(in_block->pc_unground, sourcecloudwithfeature,sourcecloudwithfeaturedown,in_block->pc_pillar, in_block->pc_beam,
