...
//负责更新局部地图并执行一些处理步骤，如特征提取、动态物体移除、下采样、边界计算等
bool MapManager::update_local_map(cloudblock_Ptr local_map, cloudblock_Ptr last_target_cblock,bool downornot,
                                  ...
                                  float dynamic_dist_thre_max, float near_dist_thre, bool recalculate_feature_on)
{
    ...
    tran_target_map = last_target_cblock->pose_lo.inverse() * local_map->pose_lo;//姿态转换,这是两个imu之间的转化，下面变成lidar到lidar的
    ...
    // lidar_to_imu(3,2)=0;
    // lidar_to_imu(3,3)=1;
    tran_target_map = lidar_to_imu.inverse()*tran_target_map*lidar_to_imu;
    //M3D Lidar_R_wrt_IMU(Eye3d); M3D Lidar_T_wrt_IMU(Eye3d);//lidar下，imu上

    //to avoid building kd-tree twice , do the map based filtering at first before the transforming，为了不重复构建kd-tree，先在变换前进行基于地图的过滤
    last_target_cblock->transform_feature(tran_target_map.inverse(), true, false);//选的的down，将target的点云转换到map的imu位姿里面了，
    ...
    if (downornot){
        local_map->append_feature(*last_target_cblock, true, used_feature_type);}//添加target的特征点，是down之后的,现在是lidar坐标系
    else{
        local_map->append_feature(*last_target_cblock, false, used_feature_type);}
    
    //we only use the undown (regarded as target) feature points for local map
    local_map->transform_feature(tran_target_map, false);//transform到target的坐标系，再转换到当前帧的雷达位姿

    local_map->pose_lo = last_target_cblock->pose_lo;//这里是当前帧imu位姿
    ...
    cf.dist_filter(local_map->pc_vertex, local_map_radius);//这个可以保证update之后是在雷达中心点附近的点云，
    ...
    cf.random_downsample_pcl(local_map->pc_vertex, kept_vertex_num);//降采样

    //calculate bbx (local)
    local_map->merge_feature_points(local_map->pc_raw, false);//集合所有点云（经过上面的下采样过后的）
    cf.get_cloud_bbx(local_map->pc_raw, local_map->local_bound);//计算边缘框

    //calculate bbx (global)
    // pcl::transformPointCloud(*local_map->pc_raw, *local_map->pc_raw_w, local_map->pose_lo*lidar_to_imu);//转换到世界坐标系（imu）下面,但是没有变换normal的位姿，需要修改
    Eigen::Matrix4d trans;
    trans=local_map->pose_lo*lidar_to_imu;
    pcl::transformPointCloudWithNormals(*local_map->pc_raw, *local_map->pc_raw_w, trans);
    cf.get_cloud_bbx(local_map->pc_raw_w, local_map->bound);

    std::chrono::steady_clock::time_point toc_2 = std::chrono::steady_clock::now();

    if (recalculate_feature_on) //Re-calculation primary vector and filter some of the points in the local map at the same time重新计算特征
    {
                                      ...
                                      int k_min, float sin_low, float sin_high, float min_linearity)
{
    ...
    for (int i = 0; i < feature_pts->points.size(); i++)
    {
        if (pca_features[i].pt_num >= k_min && pca_features[i].linear_2 > min_linearity)
        {
            ...
            {
                feature_pts->points[i].curvature = pca_features[i].linear_2; //use the lineraity to overwrite the curvature property (originally timestamp) //TODO: fix这里是用线性度去重写的curvature
